...
struct video_source *libcamera_source_create(const char *devname)
{
...

	{
		...
		if (infoMap.find(&controls::AfMode) != infoMap.end()) {
			...
			src->controls.set(controls::AfMode, controls::AfModeAuto);
		}
		if (infoMap.find(&controls::Saturation) != infoMap.end()) {
			std::cout << "Set Saturation to 0 for grayscale image" << std::endl;
			src->controls.set(controls::Saturation, (float)0.0);
		}
